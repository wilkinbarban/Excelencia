...
def generate_portuguese_thesis(output_path):
    ...
    doc.add_heading("3.7 Sistema CRM Sofia: Estratégia Transversal e Retenção", level=2)
    add_p(doc, "O CRM Sofia não é um mero cadastro de endereços, mas a espinha dorsal de inteligência comercial do negócio. Suas funções estratégicas englobam:")
    add_bullets(doc, [
        "Disparo Automatizado de Pré-Venda às Sextas-feiras e Vésperas de Feriados: Mensagens personalizadas enviadas para a base de clientes consentidos com o cardápio e botão de reserva direta.",
        "Segmentação RFM (Recência, Frequência e Valor Monetário): Identificação de clientes VIP (frequência semanal), clientes ocasionais (quinzenais) e clientes em risco de churn (inativos há mais de 35 dias).",
        "Campanhas de Reativação Inteligente: Envio de cupons de incentivo para clientes que não realizam pedidos há mais de um mês, recuperando o relacionamento sem degradar a margem geral.",
        "Gestão de Consentimento e Privacidade (LGPD): Módulo nativo de opt-in e opt-out automático ('Digite SAIR para não receber mais novidades'), armazenamento criptografado e estrita observância da Lei Geral de Proteção de Dados."
    ])
    ...
    doc.add_heading("4.7 Arquitetura Tecnológica do CRM Sofia", level=2)
    add_p(doc, "A infraestrutura do CRM Sofia foi desenhada com base em princípios de alta disponibilidade, segurança e baixo custo de manutenção: (a) Servidor Virtual Privado (VPS) em nuvem nacional com backup automatizado diário criptografado; (b) Banco de dados relacional (PostgreSQL) com modelagem orientada a entidades de clientes, pedidos e histórico de consumo; (c) Interface Web KDS (Kitchen Display System) para visualização clara dos pedidos pela equipe de cozinha; (d) Módulo de integração de mensageria com autenticação e trilha de auditoria completa.")
