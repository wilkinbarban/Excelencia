...
def build_styled_table(doc, headers, rows, widths=None, font_size=9, align_right_cols=None):
    ...
    t.autofit = False
    set_table_borders(t, color="B0B0B0", sz="6")
    ...
    for i, h in enumerate(headers):
        ...
        p.alignment = WD_ALIGN_PARAGRAPH.RIGHT if i in align_right_cols else (WD_ALIGN_PARAGRAPH.CENTER if len(str(h)) < 15 else WD_ALIGN_PARAGRAPH.LEFT)
        for r in p.runs:
            r.bold = True
            r.font.name = "Arial"
...
def add_figure_with_caption(doc, img_path, fig_num, title, source, is_pt, width_cm=15.5):
    ...
    label_prefix = f"Figura {fig_num} – " if is_pt else f"Figura {fig_num} – "
